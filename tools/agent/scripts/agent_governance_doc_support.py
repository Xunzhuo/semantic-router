...
def validate_adr_inventory_and_template(repo_manifest: dict, errors: list[str]) -> None:
    ...
    duplicate_adr_indices = sorted(
        {adr_index for adr_index in adr_indices if adr_indices.count(adr_index) > 1}
    )
...
def validate_single_adr_doc(
    ...
    if not heading_index:
        errors.append(f"ADR '{adr_doc}' must start with '# ADR 0001: Title'")
    elif filename_index and filename_index != heading_index:
        errors.append(
            f"ADR '{adr_doc}' filename index '{filename_index}' must match heading index '{heading_index}'"
        )
    return heading_index or filename_index
    ...
    duplicate_plan_indices = sorted(
        {
            ...
            if plan_indices.count(plan_index) > 1
        }
    )
...
def validate_single_plan_doc(
    ...
    plan_index = filename_match.group(1) if filename_match else ""
